...
import org.jetbrains.bazel.languages.projectview.ProjectViewCodeInsightProvider
...
internal class TargetCompletionProvider : CompletionProvider<CompletionParameters>() {
  override fun addCompletions(
    ...
    result.withPrefixMatcher(ProjectViewPrefixMatcher(prefix)).run {
      addAllElements(
        project.service<ProjectViewCodeInsightProvider>().allTargetsAndLibrariesLabels.map { labelLookupElement(it) }
          .toList(),
      )
    }
  }
